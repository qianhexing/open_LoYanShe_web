...
const previewImage = (index: number) => {
  if (!info.value?.album_foreign?.cover) return
  const images = info.value.album_foreign.cover.map(img => BASE_IMG + img)
  // 这里可以使用图片预览组件，暂时使用浏览器默认预览
  window.open(images[index], '_blank')
}

// 显示新增弹窗
const showModel = (item: { album_id: number }) => {
  show.value = true
  type.value = 0
...
// 显示修改弹窗
const editModel = (item: { album_id: number }) => {
  show.value = true
  type.value = 1
  album_id.value = item.album_id
  user_id.value = null
...
// 显示成就展示弹窗
const showAchieve = (item: { album_id: number; user_id?: number }) => {
  show.value = true
  type.value = 2
  album_id.value = item.album_id
